function AddMod()
    if mods_download then
        for k, v in pairs(mods_download) do
            TheSim:SubscribeToMod("workshop-" .. v)
        end
    end
    ...
    function EnableMod()
        if mods then
            for k, v in pairs(mods) do KnownModIndex:Enable("workshop-" .. v) end
        end
    end
end

_G.MakeRepairable = function(inst, material)
    local material = string.upper(material)
    ...
    inst.components.repairable.repairmaterial = material
end


_G.GetCurrentAnimation = function(inst)
    if _G.IsConsole() then
        print("This does not work on consoles!")
        return
    end

    local inst = (inst or ConsoleWorldEntityUnderMouse()) or nil
    if inst ~= nil then
...
_G.GetBank = function(inst)
    ...
    local inst = (inst or ConsoleWorldEntityUnderMouse()) or nil
    if inst ~= nil then
        return string.match(inst.entity:GetDebugString(), "bank: ([^ ]+) ")
    end



    return string.match(input.entity:GetDebugString(), "bank: ([^ ]+) ")
end
...
function RequiredMod(mod, RequiredMod)
    ...
    MODREQUIRED =
    {
        DEFAULT = "Please make sure to enable the " ..
        KnownModIndex:GetModFancyName("workshop-" .. mod) ..
        " to make the main mod work properly. Enable button will only work if the mod is installed",
        ...
        AddClassPostConstruct("screens/redux/multiplayermainscreen", function(self)
            function Info()
                ...
                TheFrontEnd:PushScreen(
                    PopupDialogScreen("Automatic Download",
                        "A required workshop-" .. mod .. " mod will be automatically downloaded",
                        {
                            {
                                text = "Okay 󰀩",
                                cb = function()
            ...
            if not IsOffline then
                self.inst:DoTaskInTime(.50, function()
                    ...
                    TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/Together_HUD/collectionscreen/mysterybox/LP",
                        "notif")
                    TheFrontEnd:GetSound():SetVolume("FEMusic", 0.02)

                    TheFrontEnd:GetSound():PlaySound(notif_sounds[math.random(1, #notif_sounds)])
                    TheFrontEnd:PushScreen(
                        PopupDialogScreen("Download mod?",
                            "A required workshop-" ..
                            mod .. " mod was found missing. Do you want it to be downloaded automatically?",
                            {
                                {
                                    text = "Okay 󰀃",
                                    cb = function()
