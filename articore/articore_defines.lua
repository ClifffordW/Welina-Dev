...
function GetRenderingAPI()
	-- Check if DXVK (Vulkan) is installed
	...
	local dxvkDLLPath = _G.APP_ARCHITECTURE == "x64" and "../bin64/d3d9.dll" or "../bin/d3d9.dll"
	local isDXVKInstalled = io.open(dxvkDLLPath, "r") ~= nil

	-- Set the rendering API based on DXVK presence
	local RenderingAPI
	if isDXVKInstalled then
		RenderingAPI = "Vulkan"
	else
		RenderingAPI = "DX9"
	end
...
function ShowRenderingAPI()
	if jit == nil then
		return
	end

	local RenderingAPI = GetRenderingAPI()

	local TEMPLATES = require("widgets/redux/templates")
	local old_getbuildstring = TEMPLATES.GetBuildString
	TEMPLATES.GetBuildString = function()
		return (old_getbuildstring() or "") .. " (" .. RenderingAPI .. ")"
	end
end
...
allnomain =
	{ "options", "mods", "serverlisting", "servercreation", "morgue", "collection", "mysterybox", "playersummary" }
