...
local assets = {
	...
	--Asset("SCRIPT", "scripts/prefabs/skilltree_wildcard.lua")
}
...
TUNING.GAMEMODE_STARTING_ITEMS.DEFAULT.WILDCARD = {
	"wcard_diary",
	"nightmarefuel",
	"nightmarefuel",
	"nightmarefuel",
...
for k, v in pairs(TUNING.GAMEMODE_STARTING_ITEMS) do
	start_inv[string.lower(k)] = v.WILDCARD
end
...
local function OnUpdate(inst)
    if inst.components.sanity.inducedinsanity and TUNING.WILDCARD_MENTALFATIGUE == 1 then
        ...
        if inst.sanityCountdown <= 0 then
            ...
            inst:PushEvent("death", { cause = "wildcard_fatigue" })
            inst.sanityCountdown = nil
        end
		if not inst.SoundEmitter:PlayingSound("cdown") and inst.sanityCountdown <= 300  then
			inst.SoundEmitter:PlaySound("wildcard_cd/music/wcard_countdown", "cdown")
		end
...
local function onbecamehuman(inst)
	...
	inst.components.locomotor:SetExternalSpeedMultiplier(inst, "wildcard_speed_mod", 1.0)
end

local function onbecameghost(inst)
	-- Remove speed modifier when becoming a ghost
   inst.components.locomotor:RemoveExternalSpeedMultiplier(inst, "wildcard_speed_mod")
end
--]]
...
local function TurnRabbit(inst, target)
	...
	--if not inst.components.skilltreeupdater:IsActivated("wildcard_intimidating_1") then return end
...
local common_postinit = function(inst)
	-- Minimap icon
	inst.MiniMapEntity:SetIcon("wildcard.tex")
...
local master_postinit = function(inst)
	...
	--inst.soundsname = "wildcard"
	inst.talker_path_override = "wildcard/characters/"
	...
	inst.components.locomotor:SetExternalSpeedMultiplier(inst, "wildcard_speed_mod", TUNING.WILDCARD_MOVESPEED)
	...
	inst:ListenForEvent("sanitydelta", function()
		...
		if inst.components.sanity:GetPercent() == 0 and not inst.components.sanity.inducedinsanity and TUNING.WILDCARD_BRAINTRAUMA == 1 then
			...
			inst:PushEvent("death", { cause = "wildcard_braind" })
...
--]]
	...
	inst.components.inventory.Equip = function(self, item, old_to_active)
		...
		if (item.components.equippable.equipslot == EQUIPSLOTS.HEAD and not (item:HasTag("wildcardspecial"))) then		
			self:DropItem(item)
			self:GiveItem(item)
...
return MakePlayerCharacter("wildcard", prefabs, assets, common_postinit, master_postinit, prefabs)
