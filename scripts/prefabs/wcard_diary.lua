local assets =
{
    Asset("ANIM", "anim/wcard_diary.zip"),
...
local function SanityChange(inst, owner)
    

    if owner then
        owner.components.sanity.dapperness =  TUNING.WILDCARD_DAPPERNESS / 2
    end
end
...
local function fn()
    ...
    inst.AnimState:SetBuild("wcard_diary")
    inst.AnimState:PlayAnimation("idle")
	...
	inst.components.inventoryitem.imagename = "wcard_diary"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/wcard_items.xml"

    inst:AddComponent("prototyper")
    inst.components.prototyper.restrictedtag = "horrorbunnygirl"
    inst.components.prototyper.onturnon = onturnon
    inst.components.prototyper.onturnoff = onturnoff
    inst.components.prototyper.onactivate = onactivate
    inst.components.prototyper.trees = TUNING.PROTOTYPER_TREES.WCARD_DIARY
	

    
	inst:AddComponent("wcardreadable")
    inst.components.wcardreadable.onreadfn = SanityChange
...
return Prefab("wcard_diary", fn, assets)
