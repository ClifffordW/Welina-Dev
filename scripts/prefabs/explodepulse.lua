...
local function BigWaveFX(inst)
    if ThePlayer ~= nil and ThePlayer:IsValid() then
        ThePlayer:ShakeCamera(CAMERASHAKE.SIDE, 4*FRAMES, 0.05, 4)
    end

    inst:DoTaskInTime(3*FRAMES, function(inst)
        if ThePlayer ~= nil and ThePlayer:IsValid() then
            ThePlayer:ShakeCamera(CAMERASHAKE.SIDE, 2.18, 0.12, 0.65)
        end
    end)
...
local function spawner_fn()
    ...
    if not TheNet:IsDedicated() then
        inst:DoTaskInTime(0, function()
            local pulse = SpawnPrefab("explodepulse")
            if pulse ~= nil then
                local x, _, z = inst.Transform:GetWorldPosition()
...
return Prefab("explodepulse", fn, nil, prefabs),
    Prefab("explodepulse_spawner", spawner_fn, nil, spawner_prefabs)
