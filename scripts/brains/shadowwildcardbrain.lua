...
local ShadowWildcardBrain = Class(Brain, function(self, inst)
...
function ShadowWildcardBrain:OnStart()
	...
	root = PriorityNode({
            ...
            WhileNode(function() return IsNearLeader(self.inst, KEEP_WORKING_DIST) end, "Leader In Range" ,
                PriorityNode({
                    ...
                    IfNode(function() return self.inst.prefab == "wcard_shadowduelist" end, "Is Duelist",
                        PriorityNode({
					...
					WhileNode(function() return self.inst.prefab == "wcard_shadowlumber" and not self.inst.sg:HasStateTag("phasing") end, "Keep Chopping",
                        DoAction(self.inst, function() return FindEntityToWorkAction(self.inst, ACTIONS.CHOP) end)),
					WhileNode(function() return self.inst.prefab == "wcard_shadowminer" and not self.inst.sg:HasStateTag("phasing") end, "Keep Mining",
                        DoAction(self.inst, function() return FindEntityToWorkAction(self.inst, ACTIONS.MINE) end)),
					WhileNode(function() return self.inst.prefab == "wcard_shadowdigger" and not self.inst.sg:HasStateTag("phasing") end, "Keep Digging",
